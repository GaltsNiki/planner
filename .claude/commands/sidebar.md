You will be implementing a design change to the Sidebar (side navigation panel) in this codebase.

$ARGUMENTS

ALGORITHM:

1. Use the Playwright MCP to take screenshots of the current Sidebar across the views where it appears.
2. Pass these screenshots to the Claude Code to get advice on how to upgrade this component.
3. Use the advice from Claude Code to upgrade the component, applying the DESIGN PRINCIPLES below.
...
IMPORTANT:
The Sidebar is a shared component used across the Daily, Weekly, Habits, and Overview views.
Only do this for front-end / UI changes to the Sidebar itself (navigation items, icons, active/hover states, spacing, collapse/expand behavior, etc).
Do not modify data models, business logic, or the internal content of individual views (Daily, Weekly, Habits, Overview).
Since this component is shared, changes here will affect all views — after implementing, verify the Sidebar still renders correctly in each of them.
